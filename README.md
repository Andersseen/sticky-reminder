...
<p>
  ...
  <a href="https://github.com/Andersseen/sticky-reminder/actions/workflows/deploy-cloudflare-pages.yml"><img alt="Cloudflare Pages deploy" src="https://github.com/Andersseen/sticky-reminder/actions/workflows/deploy-cloudflare-pages.yml/badge.svg"></a>
...
| [`deploy-cloudflare-pages.yml`](.github/workflows/deploy-cloudflare-pages.yml) | push to `main` touching the site, and every PR | Builds the site at the root path and publishes it to Cloudflare Pages — PRs get a preview deployment |
...
Both deploys show up under the repository's **Deployments**.

### Deploying the site

The site goes out to two hosts from the same build, differing only in where they
...
The Cloudflare deploy needs two repository **secrets**:

| Secret | Where it comes from |
|--------|---------------------|
| `CLOUDFLARE_API_TOKEN` | An API token with the **Cloudflare Pages → Edit** account permission |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare dashboard → Workers & Pages → Account ID |

and reads three optional repository **variables**:

| Variable | Default | What it changes |
|----------|---------|-----------------|
| `CLOUDFLARE_PROJECT_NAME` | `sticky-reminder` | The Pages project to deploy into |
| `CLOUDFLARE_SITE_URL` | `https://sticky-reminder.pages.dev` | The origin baked into absolute URLs — set it once a custom domain is attached |
| `SITE_CANONICAL` | unset | Two hosts serving the same pages read as duplicates. Set this to whichever origin should be indexed and both builds point `<link rel="canonical">` at it; leaving it unset makes each deploy canonical to itself |

Serving two origins is otherwise identical; [`apps/web/public/_headers`](apps/web/public/_headers)
adds caching and security headers, which only Cloudflare acts on.
